// PWA Installation and Service Worker Handler for Arabic Hub Education
class ArabicPWAHandler {
    constructor() {
    ...
    createInstallButton() {
        ...
        if (!this.installButton) {
            ...
            this.installButton.className = 'ArabicH-pwa-install-btn';
            this.installButton.innerHTML = `
                <span class="ArabicH-pwa-install-icon">📱</span>
                <span class="ArabicH-pwa-install-text" data-i18n="ArabicH.pwa.install">تثبيت التطبيق</span>
            `;
            
            // Add to header controls
            const headerControls = document.querySelector('.ArabicH-general-header-controls');
            if (headerControls) {
                ...
                this.installButton.style.left = '20px';
    ...
    updateInstallButtonText(canInstall) {
        if (this.installButton) {
            const textElement = this.installButton.querySelector('.ArabicH-pwa-install-text');
            if (textElement) {
                ...
                textElement.textContent = translations[lang] || translations.ar;
            }
        }
    }
    ...
    showUpdateNotification() {
        ...
        refreshBtn.className = 'ArabicH-pwa-refresh-btn';
        refreshBtn.innerHTML = `
            <span class="ArabicH-pwa-refresh-icon">🔄</span>
            <span class="ArabicH-pwa-refresh-text" data-i18n="ArabicH.pwa.updateAvailable">تحديث متاح</span>
        `;
    ...
    showInstallSuccessMessage() {
        ...
        successMsg.className = 'ArabicH-pwa-success-msg';
        successMsg.innerHTML = `
            <span data-i18n="ArabicH.pwa.installSuccess">تم تثبيت التطبيق بنجاح!</span>
        `;
    ...
    showCannotInstallMessage() {
        this.showTemporaryMessage('ArabicH.pwa.cannotInstall', 'var(--primary-color)');
    }

    // Show install declined message
    showInstallDeclinedMessage() {
        this.showTemporaryMessage('ArabicH.pwa.installDeclined', 'var(--accent-color)');
    }

    // Show install error message
    showInstallErrorMessage() {
        this.showTemporaryMessage('ArabicH.pwa.installError', '#ff4757');
    }

    // Helper for temporary messages
    showTemporaryMessage(translationKey, backgroundColor) {
        const msg = document.createElement('div');
        msg.className = 'ArabicH-pwa-temp-msg';
        msg.innerHTML = `<span data-i18n="${translationKey}">التثبيت غير متاح</span>`;
        msg.style.cssText = `
            position: fixed;
            top: 20px;
            right: 20px;
    ...
    addPwaTranslations() {
        ...
        Object.keys(pwaTranslations).forEach(key => {
            ...
            Object.keys(translation).forEach(lang => {
                window.currentTranslations[lang][`ArabicH.pwa.${key}`] = translation[lang];
            });
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        window.arabicPwaHandler = new ArabicPWAHandler();
    }, 1000);
});
...
.ArabicH-pwa-refresh-btn:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 15px rgba(253, 203, 110, 0.4);
}

.ArabicH-pwa-refresh-icon {
    font-size: 1.1rem;
...
[data-theme="dark"] .ArabicH-pwa-install-btn {
    background: linear-gradient(135deg, #7c6ef7 0%, #6c5ce7 100%);
}

[data-theme="dark"] .ArabicH-pwa-refresh-btn {
...
.ArabicH-pwa-install-btn.floating {
    position: fixed;
    bottom: 20px;
    left: 20px;
    z-index: 1000;
